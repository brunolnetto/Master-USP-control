...
% Platform joints Bi
B1 = point(T031, di(L21));
B2 = point(T032, di(L22));
B3 = point(T033, di(L23));

% Vectos for th1i solution
t1t2psym = sym('p', [1, 9]);
posrel = [x(t), y(t), alpha(t), ...
...
eqn_em = subs(eqn_em, t1t2psym, posrel);
...
eqn_phi = subs(eqn_phi, t1t2psym, posrel);
...
v11_cg = velocity(T021p, di(L11_cg));
v12_cg = velocity(T022p, di(L12_cg));
v13_cg = velocity(T023p, di(L13_cg));

v21_cg = velocity(T031p, di(L21_cg));
v22_cg = velocity(T032p, di(L22_cg));
v23_cg = velocity(T033p, di(L23_cg));

ve_cg = velocity(T0ep, point(delta_e_cg, di(Le_cg)));
...
D11t = S*Rbq1*DG1;
D12t = zeros(6, 6);
D21t = [S*Rbq1q2*DG2; zeros(2, 3); eye(3); eye(3); zeros(1, 3)];
