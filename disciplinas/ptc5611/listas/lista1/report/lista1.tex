...
\usepackage{graphics, graphicx,fancyhdr, subfig,  color, float}
...
% Evita que títulos sejam exibidos no final da pagina
\usepackage[nobottomtitles*]{titlesec}

% Cores definidas pelo usuario
\definecolor{mygreen}{rgb}{0.133, 0.545, 0.133} % color values Red, Green, Blue
\definecolor{mylilas}{rgb}{0.627, 0.126,0.941}
\definecolor{myblue}{rgb}{0, 0, 1}

% Configuracao da linguagem MatLab para exibicao
\lstset{
    language=Matlab,
    tabsize = 4,
    framesep = 3mm,
...
\section*{Exercício 1}
    ...
    \begin{enumerate}
        ...
        \item %B
        Como demonstrado no item \ref{item:ex1a}, conclui-se que $f_s = 12$ $kHz$. Dada frequência do sistema de $4000\pi \frac{rad}{s}$, pelo teorema de Nyquist $\omega_s \stackrel{!}{>} 8000\pi \frac{rad}{s} \approx 24000 \frac{rad}{s}$. Devido à violação, não é possível reconstruir o sinal por meio de um filtro passa-baixas ideal. 
...
\section*{Exercício 2}
    ...
    \begin{enumerate}
        ...
        Logo, para \eqref{eq:appprop1} obtemos:
            ...
            \begin{equation}
                ...
                \end{split}
            \label{eq:appprop_resolvido}
            \end{equation}
        
        Consequentemente, com o resultado de \eqref{eq:appprop_resolvido} e \eqref{eq:appprop2}, tem-se:
        ...
        Por fim, a expressão resultante é:
    ...
    Seja o pólo $s=0$ numericamente mapeado, entretanto, para $s_0 = -0.01$ em espaço $s$, o respectivo pólo em $z$ equivale a $z_0 = e^{T_s s_0}$, com $T_s = 0.01$ s. Assim $z_0 \approx 0.999$.
        ...
        Perceba que a função de transferência resultante é biprópria.
    \end{enumerate}

\section*{Exercício 4}

    A fim de garantir a mesma energia à entrada degrau, a seguinte equação deve ser satisfeita:
    ...
    Pelo método de expansão de frações parciais, a equação (\ref{eq:ex41}) apresenta a seguinte forma
        ...
        \begin{equation}
            H(s) = \frac{A}{s} + \frac{B}{(s+a)} \mbox{, com }
            \left\{\begin{cases}
            A &= s H(s) \bigg\rvert_{s=0} = \frac{K}{a} \\
            B &= (s+a) H(s) \bigg\rvert_{s=-a} = -\frac{K}{a}
            \end{cases}\right.
        \end{equation}
    ...
    Portanto, a transformada $\mathcal{Z}$ do sinal (\ref{eq:lapinv}) é dada por
    ...
    Por fim, por meio da função de transefrência de uma malha fechada \footnote{A equação em malha fechada de um sistema em tempo discreto ou contínuo é dado pela equação $G_{MF}(z) = \frac{G(z)}{1+G(z)}$}, após manipulação algébrica é dada por:
        ...
        \begin{equation}
            G_{MF}(z) = K' \frac{z}{z+b} \mbox{, com } K' = \frac{K}{a} (1-e^{-aT_s}) \mbox{ e } b = \left(1 + \frac{K}{a}\right)e^{-aT_s} - \frac{K}{a} 
        \end{equation}
...
\label{sec:ex6}
    ...
    Para os critérios de projeto fornecidos, o projeto de controle será feito em espaço $s$ e o respectivo pólo deve pertencer ao lugar das raízes em malha fechada. 
...
\section*{Exercício 7}
    ...
    Seja $\mathbf{a} \in \mathbb{R}^n$, $\mathbf{b} \in \mathbb{R}^m$ e $\mathbf{z}_k = \left[z^{-k}, \cdots, 1\right]^T$, a equação de diferenças de $G(z^{-1}) = \frac{Y(z^{-1})}{U(z^{-1})} = \frac{\textbf{b}^T \mathbf{z}_{m}}{a^T \mathbf{z}_{n}}$ é, por definição, descrita em tempo discreto por $\textbf{a}^T \textbf{y}_{m}[k] = \textbf{b}^T \textbf{u}_{n}[k]$. Os requisitos são obter uma função \texttt{tf2diff(G, u, y)}, com \texttt{G} a função de transferência $G$, \texttt{y} os $n$ valores anteriores da saída $y$ e \texttt{u} os $m$ valores anteriores da entrada $u$ e o valor atual $u[k]$. O vetor de entrada $\texttt{u} = \mathbf{u}_0^T[k] = \left[u[k - m], \cdots, u[k]\right]$ e saídas $\texttt{y} = \mathbf{y}_0^T[k] = \left[y[k - n], \cdots,\right.$ $\left.y[k - 1]\right]$, $\dim(\texttt{y}) = n$ e $\dim(\texttt{u}) = m + 1$. 
    
    Desta forma, a equação de diferenças resultante é
    ...
    \begin{equation}
        \textbf{F} = \begin{bmatrix} \textbf{I}_{n} \\ \textbf{0} \\ \end{bmatrix}_{n+1, n}
    \end{equation}
...
\section*{Preparação para os exercícios 8 e 9}
\label{sec:deducao89}
    ...
    Como $a b^T = M \,\, M \in \mathbb{R}^{2x2}$, a equação torna-se \eqref{eq:sexpi} torna-se:
    
    \begin{equation}
        s^i = \frac{c^T M_c^{i - 1} \, \mathcal{Z}}{d^T \, M_d^{i - 1} \, \mathcal{Z}}
        \label{eq:smi}
    \end{equation}
    
    Por definição, $M = V \Lambda V^{-1}$, $\forall M \in \mathbb{R}^{n \times n}$, com $V$ a matriz direita de autovetores de $M$ e $\Lambda$ a matriz de autovalores de $M$, diagonal por definição. Além disso, $f(M) = V f(\Lambda) V^{-1}$. Em especial, $M^i = V \Lambda^i V^{-1}$. Assim, a equação (\ref{eq:smi}) reduz-se a:
    ...
    \begin{equation}
        \mathcal{S}_k^T = \left[\frac{c^T V_c \Lambda_c^{k-1} V_c^{-1} \mathcal{Z}}{d^T V_d \Lambda_d^{k-1} V_d^{-1} \mathcal{Z}}, \cdots, \frac{c^T V_c \Lambda_c^{-1} V_c^{-1} \mathcal{Z}}{d^T V_d \Lambda_d^{-1} V_d^{-1} \mathcal{Z}}\right]
    \end{equation}
...
\section*{Exercício 8}
    ...
    A transformação para trás tem parâmetros $c^T = [1, -1]$ e $d^T = [T_s, 0]$ e a transformação para frente $c^T = [1, -1]$ e $d^T = [0, T_s]$. As implementações em MatLab estão nas subseções \ref{subsec:s2z}, \ref{subsec:backward} e \ref{subsec:forward}.

\section*{Exercício 9}
\label{ex:ex9}

    A transformação de Tustin tem parâmetros $c^T = [2, 2]$ e $d^T = [T_s, -T_s]$. A implementação em MatLab encontra-se nas subseções \ref{subsec:s2z} e \ref{subsec:tustin}.
...
\section{Scripts em MatLab}

    \subsection{Script do exercício 6}
    \label{subsec:ex6}
    \lstinputlisting[caption = Constantes e comandos para item A e B do \sectionautorefname{sec:ex6}]{code/ex6.m}

    \subsection{Simbólicos para função de transferência}
    \label{subsec:syms2tfz}
    \lstinputlisting[caption = Conversão de simbólico fracionário em função de transferência discreta]{code/syms2tfz.m}
    
    \subsection{Transformação de S para Z}
    \label{subsec:s2z}
    \lstinputlisting[caption = Conversão de função de transferência por transformação $T(z)$]{code/s2z.m}
    
    \subsection{Transformação de z para s para trás}
    \label{subsec:backward}
    \lstinputlisting[caption = Transformação de conversão para trás]{code/backward.m}
    
    \subsection{Transformação de z para s para frente}
    \label{subsec:forward}
    \lstinputlisting[caption = Transformação de conversão para frente]{code/forward.m}
    
    \subsection{Transformação de z para s tustin estritamente própria}
    \label{subsec:tustin}
    \lstinputlisting[caption = Transformação de conversão tustin própria]{code/tustin_prop.m}
