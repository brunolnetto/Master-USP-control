...
De mesma forma, dada função $x_2(t) = \frac{1}{a} \left( 1 - e^{-at} \right)$, por definição tem-se que:
	...
	& = \sum_{i=0}^{\infty} T_s^i i^2 e^{-a T_s i} z^{-i}
	\end{split}
	\end{equation}
...
\begin{eqnarray}
x[n] = e^{-an} \rightarrow -z \frac{d}{dz} \mathcal{e^{-an}} = $\mathcal{Z}$\{n e^{-an}\}\\
x[n] = n e^{-an} \rightarrow -z \frac{d}{dz} \mathcal{n e^{-an}} = $\mathcal{Z}$\{n^2 e^{-an}\}
\end{eqnarray}
